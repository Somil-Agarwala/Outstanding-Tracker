...
export default function Dashboard() {
  ...
  const stats = useMemo(() => {
    /*
      CORRECT DEFINITIONS (these match across dashboard, invoices page, and reports):

      net_outstanding  = invoice_amount + cn_dn_amount   (total invoice value)
      payment_received = money actually collected so far
      balance          = net_outstanding - payment_received  (computed by DB trigger)
      outstanding      = sum of balance WHERE balance > 0   (unpaid money still owed)
      collected        = sum of payment_received            (cash received)
      overdue          = balance > 0 AND past due date
    */
    const outstanding    = invoices.reduce((s, i) => s + Math.max(0, Number(i.balance    ?? 0)), 0)
    const collected      = invoices.reduce((s, i) => s + Number(i.payment_received        ?? 0), 0)
    const totalPDC       = invoices.reduce((s, i) => s + Number(i.pdc_amount              ?? 0), 0)

    const overdueInvs    = invoices.filter(i => callStatus(i) === 'overdue')
    const overdueAmt     = overdueInvs.reduce((s, i) => s + Math.max(0, Number(i.balance ?? 0)), 0)
    ...
    // Chart: unpaid balance + overdue split by company
    const byCompany = {}
    invoices.forEach(inv => {
      ...
      byCompany[k].outstanding += bal
      if (callStatus(inv) === 'overdue') byCompany[k].overdue += bal
    })
    ...
    invoices.forEach(inv => {
      ...
      const bal = Math.max(0, Number(inv.balance ?? 0))
      if (!byLocation[l]) byLocation[l] = {
        outstanding: 0, overdue: 0, collected: 0, count: 0, callDue: 0,
      }
      byLocation[l].outstanding += bal
      byLocation[l].collected   += Number(inv.payment_received ?? 0)
      byLocation[l].count++
      if (callStatus(inv) === 'overdue') byLocation[l].overdue += bal
      if (callStatus(inv) === 'call_due' || callStatus(inv) === 'due_today')
        byLocation[l].callDue++
    })
    const locationRows = Object.entries(byLocation)
      .sort((a, b) => b[1].outstanding - a[1].outstanding)

    return {
      outstanding, collected, totalPDC,
  ...
  return (
    <div className="p-5 space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 xl:grid-cols-7 gap-3">
        <KPI label="Outstanding"  value={fmtCurrency(stats.outstanding)} sub="unpaid balance"              color="indigo" />
        ...
        <KPI label="Collected"    value={fmtCurrency(stats.collected)}   sub="payments received"           color="green" />
                ...
                {stats.overdueInvs.slice(0, 10).map(inv => {
                  ...
                  return (
                    <tr key={inv.id}>
                      ...
                      <td className="td"><span className="badge badge-red">{delay}d late</span></td>
                    </tr>
                  )
                })}
              </tbody>
